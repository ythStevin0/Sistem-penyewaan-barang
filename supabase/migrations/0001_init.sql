...
CREATE TYPE rental_status AS ENUM ('menunggu_pembayaran', 'dibayar', 'diambil', 'selesai', 'dibatalkan', 'terlambat');
...
CREATE POLICY "Users can view their own data" ON public.users FOR SELECT USING (auth.uid() = id OR public.is_admin());
CREATE POLICY "Users can update their own data" ON public.users FOR UPDATE USING (auth.uid() = id OR public.is_admin());
...
CREATE POLICY "Categories are insertable by admins" ON public.categories FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "Categories are updatable by admins" ON public.categories FOR UPDATE USING (public.is_admin());
CREATE POLICY "Categories are deletable by admins" ON public.categories FOR DELETE USING (public.is_admin());

-- Products Policies
CREATE POLICY "Products are viewable by everyone" ON public.products FOR SELECT USING (true);
CREATE POLICY "Products are insertable by admins" ON public.products FOR INSERT WITH CHECK (public.is_admin());
...
-- Rentals Policies
CREATE POLICY "Users can view their own rentals" ON public.rentals FOR SELECT USING (auth.uid() = user_id OR public.is_admin());
CREATE POLICY "Users can create rentals" ON public.rentals FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own rentals" ON public.rentals FOR UPDATE USING (auth.uid() = user_id OR public.is_admin());
CREATE POLICY "Admins can view all rentals" ON public.rentals FOR SELECT USING (public.is_admin());
CREATE POLICY "Admins can update all rentals" ON public.rentals FOR UPDATE USING (public.is_admin());

-- Rental Items Policies
CREATE POLICY "Users can view their own rental items" ON public.rental_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.rentals WHERE id = rental_id AND user_id = auth.uid()) OR public.is_admin()
);
CREATE POLICY "Users can create rental items" ON public.rental_items FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.rentals WHERE id = rental_id AND user_id = auth.uid())
);
CREATE POLICY "Admins can view all rental items" ON public.rental_items FOR SELECT USING (public.is_admin());
...
CREATE POLICY "Users can create reviews for their rentals" ON public.reviews FOR INSERT WITH CHECK (auth.uid() = user_id);
